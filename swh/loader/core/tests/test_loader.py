...
class TestLoader(unittest.TestCase):
    ...
    def shallow_blob(self):
        # when
        actual_blob = loader.shallow_blob({
            'length': 1451,
            'sha1_git':
    ...
    def shallow_tree(self):
        # when
        actual_shallow_tree = loader.shallow_tree({
            'length': 1451,
            'sha1_git':
            b'tree-id',
            'type': b'tree',
    ...
    def shallow_commit(self):
        # when
        actual_shallow_commit = loader.shallow_commit({
            'sha1_git':
            b'\xd1\xdd\x9a@\xeb\xf6!\x99\xd4[S\x05\xa8Y\xa3\x80\xa7\xb1;\x9c',
            'type': b'commit',
    ...
    def shallow_tag(self):
        # when
        actual_shallow_tag = loader.shallow_tag({
            'sha1':
            b'\xd1\xdd\x9a@\xeb\xf6!\x99\xd4[S\x05\xa8Y\xa3\x80\xa7\xb1;\x9c',
            'type': b'tag',
