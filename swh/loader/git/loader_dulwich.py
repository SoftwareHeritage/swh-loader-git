...
# ^ This is a preserved copy of the pre-rehaul dulwich-based GitLoader
# (commit 4ea30e3). Dulwich's type annotations evolved (Ref/ObjectID
# aliases vs bytes); the 5 arg-type errors are cosmetic — the runtime
# types are identical (bytes). Suppressing rather than refactoring to
# keep the copy-and-rename contract (no behavioral changes).
...
class GitLoaderDulwich(BaseGitLoader):
    """Dulwich-based fallback loader for pathological repositories.

    This is a copy of the pre-rehaul (dulwich-based) GitLoader, preserved
    verbatim for use as a fallback backend when the gix-based loader
    rejects a pack/object that dulwich is historically tolerant of.

    Source: ``git show 4ea30e3:swh/loader/git/loader.py`` (the last commit
    before the gitoxide rehaul landed on ``feat/gix-phase1-fetch``).

    Design decision: copy-and-rename was chosen over wrapping
    ``GitLoaderFromDisk`` to preserve exact historical behavior.
    See ``PLAN-dulwich-fallback-wiring.md §13.1``.
