...
def _entries_to_directory_entries(
    ...
    validators.  Benchmarks (notes/PLAN-tree-bottleneck.md, Phase B) show
    this is ~2.7x faster than the regular ``DirectoryEntry(...)`` constructor
    path — 0.32 us/entry vs 0.81 us/entry on a typical workload.  Validation
    is safe to skip here: ``type`` is derived from the mode mask (always
    one of the three valid values), ``target`` and ``name`` come from gix's
    tree parser which already validated them.
    """
    entries = []
...
def tree_to_directory_preparsed(
    ...
    (notes/PLAN-tree-bottleneck.md, Phase B).
    """
