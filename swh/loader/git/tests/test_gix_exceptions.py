...
See ``gix-py/src/exceptions.rs`` for the class hierarchy and
``notes/DESIGN-dulwich-fallback-signals.md`` in the swh-environment audit
tree for the design rationale. The classes let the SWH git loader's
dispatch layer decide per-exception-class whether to re-dispatch to the
dulwich-based fallback worker.
"""
