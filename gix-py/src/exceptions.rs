...
//! `notes/DESIGN-dulwich-fallback-signals.md` §6 and
//! `memory/project_dulwich_fallback_decision.md` of the swh-environment
//! audit tree. Before this change, gix-py wrapped every anyhow error as a
//! bare `PyValueError` whose message was the stringified error chain; a
//! dispatch predicate would have had to string-match on that message, a
//! coupling that gitoxide does not commit to preserving across minor
//! releases.
//!
